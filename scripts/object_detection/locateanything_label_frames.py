...
def load_categories(path: Path | None) -> list[str]:
    ...
    if not isinstance(categories, list) or not all(isinstance(item, str) for item in categories):
        raise ValueError(f"Invalid categories file: {path}")
    return categories
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Label per-video frame folders with LocateAnything.")
    ...
    categories = load_categories(Path(args.categories_file) if args.categories_file else None)
